    ...
    <>
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-4xl font-bold text-[#333] mb-12">{lifeSummary.headline}</h2>
          <div className="flex flex-col md:flex-row justify-center gap-8">
            <div className="border-4 border-red-500 rounded-lg p-6 flex-1">
              <h3 className="text-2xl font-bold text-red-500 mb-4">{lifeSummary.cookingAlone.title}</h3>
              <ul className="space-y-2 text-left">
                {lifeSummary.cookingAlone.items.map((item, index) => (
                  <li key={index} className="flex items-start"><span className="text-red-500 mr-2">✘</span>{item}</li>
                ))}
              </ul>
            </div>
            <div className="border-4 border-green-500 rounded-lg p-6 flex-1">
              <h3 className="text-2xl font-bold text-green-500 mb-4">{lifeSummary.cookingWithMarmitasJa.title}</h3>
              <ul className="space-y-2 text-left">
                {lifeSummary.cookingWithMarmitasJa.items.map((item, index) => (
                  <li key={index} className="flex items-start"><span className="text-green-500 mr-2">✔</span>{item}</li>
                ))}
              </ul>
            </div>
          </div>
      ...
      <section className="py-12 bg-[#7ED957]">
        <h2 className="text-4xl font-extrabold text-white text-center">{awayFromTheStove.headline}</h2>
      </section>

      <section id="final-cta-section" className="py-16 bg-[#F5F5F5]">
        <div className="container mx-auto px-4 text-center">
          <div className="flex justify-center items-center gap-4 mb-4">
            <img src={finalCta.image1} alt="" className="w-20 h-auto" />
            <img src={finalCta.image2} alt="" className="max-w-md h-auto" />
          </div>
          <p className="text-2xl text-gray-500 line-through">{finalCta.oldPrice}</p>
          <p className="text-2xl text-[#333]">{finalCta.finalPriceLabel}</p>
          <p className="text-6xl font-extrabold text-[#7ED957] my-2">{finalCta.finalPrice}</p>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-4xl font-bold text-[#333]">{sevenDayGuarantee.headline1} <span className="text-[#7ED957]">{sevenDayGuarantee.headline2}</span></h2>
          <img src={sevenDayGuarantee.image} alt="" className="mx-auto my-8 w-48 h-48" />
          <p className="text-lg text-gray-700 max-w-2xl mx-auto">{sevenDayGuarantee.text}</p>
        </div>
      </section>
